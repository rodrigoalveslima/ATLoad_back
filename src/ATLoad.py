...
class Session:
  ...
  def _run(self, conf, start_time, start_at, stop_at, request_graph, mean_think_time, think_time_generator, intensity):
    self._logs = []
        ...
        if isinstance(intensity, list):
          ...
          if intensity[window_no] != 1:
            ...
            if (window_no == 0 or intensity[window_no - 1] == 1) and elapsed < window_half:
              think_time -= 0.01 * (intensity[window_no] - 1) * ((elapsed - window_start) / (conf["burstiness"]["window"] / 2))
            elif intensity[window_no + 1] == 1 and elapsed > window_half:
              think_time -= 0.01 * (intensity[window_no] - 1) * ((window_end - elapsed) / (conf["burstiness"]["window"] / 2))
            else:
...
class Workload:
  ...
  def __init__(self, conf_filename, log_filename, session_cls, random_seed, n_workers, *args):
    ...
    self._request_graph = dict([(request, collections.OrderedDict(self._conf["request_graph"][request])) for request in self._conf["request_graph"]])
    self._mean_think_time = self._conf["think_time"]
    self._think_time_generator = getattr(numpy.random,
                                         self._conf["think_time_distribution"]) if self._conf["think_time_distribution"] != "constant" else lambda x: x
    if "burstiness" not in self._conf:
      ...
      for i in range(int((self._conf["duration"]["total"] + 60) / self._conf["burstiness"]["window"])):
        if i * self._conf["burstiness"]["window"] > self._conf["duration"]["ramp_up"] and \
            i * self._conf["burstiness"]["window"] < self._conf["duration"]["total"] - self._conf["duration"]["ramp_down"]:
          if i == 0 or self._intensity[-1] != 1:
            self._intensity.append(self._conf["burstiness"]["intensity"] if random.uniform(0, 1) > self._conf["burstiness"]["turn_off_prob"] else 1)
          else:
            self._intensity.append(self._conf["burstiness"]["intensity"] if random.uniform(0, 1) < self._conf["burstiness"]["turn_on_prob"] else 1)
        else:
  ...
  def _run_worker(self, log_filename, start_time, n_sessions, start_at_delta):
    ...
    threads = [
        threading.Thread(target=session._run,
                         args=[
                             self._conf, start_time, start_time + self._duration["ramp_up"] * (i / n_sessions) + start_at_delta,
                             start_time + self._duration["total"] - self._duration["ramp_down"] * (i / n_sessions), self._request_graph, self._mean_think_time,
    ...
    while True:
      session_i = None
      for (i, session) in enumerate(sessions):
        if session._logs and (session_i is None or session._logs[0][0] < sessions[session_i]._logs[0][0]):
          session_i = i
  ...
  def run(self):
    ...
    workers = [
        multiprocessing.Process(target=self._run_worker,
                                args=(os.path.join(dirname, filename + str(i) + '.' + extension), start_time, int(self._n_sessions // self._n_workers),
                                      i * self._conf["duration"]["ramp_up"] / self._n_sessions)) for i in range(self._n_workers)
    ]
